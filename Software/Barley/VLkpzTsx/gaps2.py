...
def ref_cfg(rod, wall):
    ...
    Process.Mesh__Selection__Action__Load_Selection('rods.txt', 'No', 'No')
...
def write_csv_save_mesh(a, press_gc, press_sc, file_path):
    ...
    Process.Mesh__System__Save('',file_path + '/GC_' + str(a) + '.mdxm', 'no')
...
def iterate_pressure(p_gc, p_sc, where):
    ...
    for i in range(0,len(ratio)):
        gc1 = ratio[i]*p_gc
        gc2 = p_gc - gc1
        print("P_GC1 = ", gc1, " , P_GC_2 = ", gc2)
        set_gc_pressure(gc1,'gc1.txt')
        set_gc_pressure(gc2,'gc2.txt')
        run_fem() 
        write_csv_save_mesh(i+1, gc1, gc2, where)
...
###############################
for n in range(0,len(pressure_gc)):
    
    out_dir = const.out_dir + str(pressure_gc[n])  
    make_dir(out_dir)
    ...
    print("Incrementing Guard Cell and Subsidiary Cell Pressure")
    iterate_pressure(pressure_gc[n], pressure_sc, out_dir)
    move_csv(out_dir)
